...
public class ComputeCallable<I extends WritableComparable, V extends Writable, E extends Writable, M1 extends Writable, M2 extends Writable>
  ...
  public ComputeCallable(Mapper<?, ?, ?, ?>.Context context,
    ...
    messageBytesSentCounter = metrics.getCounter(MetricNames.MESSAGE_BYTES_SENT);
    ...
    histogramWaitTimePerThread = metrics.getUniformHistogram("wait-per-thread-ms");
    histogramProcessingTimePerThread = metrics.getUniformHistogram("processing-per-thread-ms");
    verticesToUpdateProgress = GiraphConstants.VERTICES_TO_UPDATE_PROGRESS.get(configuration);
  }
  ...
  public Collection<PartitionStats> call() {
    ...
    WorkerClientRequestProcessor<I, V, E> workerClientRequestProcessor = new NettyWorkerClientRequestProcessor<I, V, E>(
        ...
        // configuration.getOutgoingMessageEncodeAndStoreType().useOneMessageToManyIdsEncoding());
        true); //use OneMessageToManyIds class to ensure privacy
    WorkerThreadGlobalCommUsage aggregatorUsage = serviceWorker.getAggregatorHandler().newThreadAggregatorUsage();

    vertexWriter = serviceWorker.getSuperstepOutput().getVertexWriter();

    Computation<I, V, E, M1, M2> computation = (Computation<I, V, E, M1, M2>) configuration.createComputation();
    ...
    while (true) {
      ...
      long timeDoingGCWhileWaiting = taskManager.getSuperstepGCTime() - startGCTime;
      ...
      try {
        ...
        long timeDoingGCWhileProcessing = taskManager.getSuperstepGCTime() - startGCTime;
        timeDoingGC += timeDoingGCWhileProcessing;
        long timeProcessingPartition = System.currentTimeMillis() - startProcessingTime -
    ...
    try {
      ...
      if (partitionStatsList.size() > 0) {
        long partitionMsgBytes = workerClientRequestProcessor.resetMessageBytesCount();
        partitionStatsList.get(partitionStatsList.size() - 1).addMessageBytesSentCount(partitionMsgBytes);
        messageBytesSentCounter.inc(partitionMsgBytes);
      }
