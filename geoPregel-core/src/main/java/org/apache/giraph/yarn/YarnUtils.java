    ...
    for (String fileName : giraphConf.getYarnLibJars().split(",")) {
      if (fileName.length() > 0) {
        ...
        LOG.info("Adding " + fileName + " to LocalResources for export.to " +
          filePath);
        if (fileName.contains("geoPregel-core")) {
          coreJarFound = true;
        }
        addFileToResourceMap(map, fs, filePath);
      }
    }
    if (!coreJarFound) { // OK if you are running geoPregel-examples-jar-with-deps
      LOG.warn("Job jars (-yj option) didn't include geoPregel-core.");
    }
