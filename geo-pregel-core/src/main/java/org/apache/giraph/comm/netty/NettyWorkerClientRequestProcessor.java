...
public class NettyWorkerClientRequestProcessor<I extends WritableComparable, V extends Writable, E extends Writable>
  ...
  private final SendMutationsCache<I, V, E> sendMutationsCache = new SendMutationsCache<I, V, E>();
  ...
  public NettyWorkerClientRequestProcessor(
    ...
    // thb:Caches partition vertices prior to sending. Aggregating these requests
    // will make larger, more efficient requests. Not thread-safe.
    // 在发送之前，缓存分区顶点。将这些请求集合起来，会产生更大、更有效的请求。不是线程安全的。
    sendPartitionCache = new SendPartitionCache<I, V, E>(conf, serviceWorker);

    // thb:Aggregates the edges to be sent to workers so they can be sent in bulk.
    // Not thread-safe.
    // 聚集要发送给工作者的边，以便它们可以被批量发送。不是线程安全的。
    sendEdgeCache = new SendEdgeCache<I, E>(conf, serviceWorker);

    maxMessagesSizePerWorker = GiraphConfiguration.MAX_MSG_REQUEST_SIZE.get(conf);
    maxVerticesSizePerWorker = GiraphConfiguration.MAX_VERTEX_REQUEST_SIZE.get(conf);

    // maxMessagesSizePerWorker = Integer.MAX_VALUE;

    // thb: Aggregates the messages to be sent to workers so they can be sent in
    // bulk.
    // 聚集要发送给工作者的信息，以便它们可以被批量发送。
    if (useOneMessageToManyIdsEncoding) {
      sendMessageCache = new SendOneMessageToManyCache<I, Writable>(conf, serviceWorker,
          this, maxMessagesSizePerWorker);
    } else {
      sendMessageCache = new SendMessageCache<I, Writable>(conf, serviceWorker,
          this, maxMessagesSizePerWorker);
    }
    maxEdgesSizePerWorker = GiraphConfiguration.MAX_EDGE_REQUEST_SIZE.get(conf);
    maxMutationsPerPartition = GiraphConfiguration.MAX_MUTATIONS_PER_REQUEST.get(conf);
  ...
  @Override
  // thb: Send a message to a target vertex id.
  // 向一个目标顶点ID发送一个信息。
  ...
  @Override
  // thb: Send message to all its neighbors
  // 发送到所有的neighbour
  // vertex - The source vertex
  // message - The message sent to a worker
  ...
  public void sendPartitionRequest(WorkerInfo workerInfo,
    ...
    if (serviceWorker.getSuperstep() != BspService.INPUT_SUPERSTEP) {
      ...
      ConcurrentMap<I, VertexMutations<I, V, E>> vertexMutationMap = serverData.getPartitionMutations()
          .remove(partition.getId());
      WritableRequest partitionMutationsRequest = new SendPartitionMutationsRequest<I, V, E>(partition.getId(),
          vertexMutationMap);
  ...
  private void sendPartitionMessages(WorkerInfo workerInfo,
    ...
    MessageStore<I, Writable> messageStore = serverData.getCurrentMessageStore();
    ByteArrayVertexIdMessages<I, Writable> vertexIdMessages = new ByteArrayVertexIdMessages<I, Writable>(
        messageValueFactory);
    vertexIdMessages.setConf(configuration);
    vertexIdMessages.initialize();
    for (I vertexId : messageStore.getPartitionDestinationVertices(partitionId)) {
      ...
      if (vertexIdMessages.getSize() > maxMessagesSizePerWorker) {
        WritableRequest messagesRequest = new SendPartitionCurrentMessagesRequest<I, V, E, Writable>(
            partitionId, vertexIdMessages);
        doRequest(workerInfo, messagesRequest);
        vertexIdMessages = new ByteArrayVertexIdMessages<I, Writable>(
            messageValueFactory);
        vertexIdMessages.setConf(configuration);
        vertexIdMessages.initialize();
      }
  ...
  public boolean sendVertexRequest(PartitionOwner partitionOwner,
    ...
    if (workerMessageSize >= maxVerticesSizePerWorker) {
      PairList<Integer, ExtendedDataOutput> workerPartitionVertices = sendPartitionCache
          .removeWorkerData(partitionOwner.getWorkerInfo());
      WritableRequest writableRequest = new SendWorkerVerticesRequest<I, V, E>(
  ...
  public void addEdgeRequest(I vertexIndex, Edge<I, E> edge) throws IOException {
    PartitionOwner partitionOwner = serviceWorker.getVertexPartitionOwner(vertexIndex);
    ...
    int partitionMutationCount = sendMutationsCache.addEdgeMutation(partitionId, vertexIndex, edge);
  ...
  public boolean sendEdgeRequest(I sourceVertexId, Edge<I, E> edge)
      throws IOException {
    PartitionOwner owner = serviceWorker.getVertexPartitionOwner(sourceVertexId);
    ...
    if (workerEdgesSize >= maxEdgesSizePerWorker) {
      PairList<Integer, VertexIdEdges<I, E>> workerEdges = sendEdgeCache.removeWorkerEdges(workerInfo);
      WritableRequest writableRequest = new SendWorkerEdgesRequest<I, E>(workerEdges);
  ...
  private void sendMutationsRequestIfFull(
    ...
    if (partitionMutationCount >= maxMutationsPerPartition) {
      Map<I, VertexMutations<I, V, E>> partitionMutations = sendMutationsCache.removePartitionMutations(partitionId);
      WritableRequest writableRequest = new SendPartitionMutationsRequest<I, V, E>(
  ...
  public void removeEdgesRequest(I vertexIndex,
    ...
    // Add the message to the cache
    int partitionMutationCount = sendMutationsCache.removeEdgeMutation(
  ...
  public void addVertexRequest(Vertex<I, V, E> vertex) throws IOException {
    PartitionOwner partitionOwner = serviceWorker.getVertexPartitionOwner(vertex.getId());
    int partitionId = partitionOwner.getPartitionId();
    if (LOG.isTraceEnabled()) {
    ...
    // Add the message to the cache
    int partitionMutationCount = sendMutationsCache.addVertexMutation(partitionId, vertex);
  ...
  public void removeVertexRequest(I vertexIndex) throws IOException {
    ...
    // Add the message to the cache
    int partitionMutationCount = sendMutationsCache.removeVertexMutation(partitionId, vertexIndex);
  ...
  public void flush() throws IOException {
    ...
    PairList<WorkerInfo, PairList<Integer, ExtendedDataOutput>> remainingVertexCache = sendPartitionCache
        .removeAllData();
    PairList<WorkerInfo, PairList<Integer, ExtendedDataOutput>>.Iterator vertexIterator = remainingVertexCache
        .getIterator();
    while (vertexIterator.hasNext()) {
      vertexIterator.next();
      WritableRequest writableRequest = new SendWorkerVerticesRequest(
    ...
    PairList<WorkerInfo, PairList<Integer, VertexIdEdges<I, E>>> remainingEdgeCache = sendEdgeCache.removeAllEdges();
    PairList<WorkerInfo, PairList<Integer, VertexIdEdges<I, E>>>.Iterator edgeIterator = remainingEdgeCache
        .getIterator();
    while (edgeIterator.hasNext()) {
      edgeIterator.next();
      WritableRequest writableRequest = new SendWorkerEdgesRequest<I, E>(
    ...
    Map<Integer, Map<I, VertexMutations<I, V, E>>> remainingMutationsCache = sendMutationsCache
        .removeAllPartitionMutations();
    for (Map.Entry<Integer, Map<I, VertexMutations<I, V, E>>> entry : remainingMutationsCache.entrySet()) {
      WritableRequest writableRequest = new SendPartitionMutationsRequest<I, V, E>(
          entry.getKey(), entry.getValue());
      PartitionOwner partitionOwner = serviceWorker.getVertexPartitionOwner(
  ...
  public void doRequest(WorkerInfo workerInfo,
    ...
    if (serviceWorker.getWorkerInfo().getTaskId() == workerInfo.getTaskId()) {
