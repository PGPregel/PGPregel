...
public class SendMessageCache<I extends WritableComparable, M extends Writable>
  ...
  public SendMessageCache(ImmutableClassesGiraphConfiguration conf,
    ...
    messageValueFactory = conf.createOutgoingMessageValueFactory();
  }
  ...
  public void sendMessageRequest(I destVertexId, M message) {
    PartitionOwner owner = getServiceWorker().getVertexPartitionOwner(destVertexId);
    ...
    // thb:这里有一个map映射的关系，返回的是map[workerINFO]的cache大小
    int workerMessageSize = addMessage(
        workerInfo, partitionId, destVertexId, message);
    // Send a request if the cache of outgoing message to
    // the remote worker 'workerInfo' is full enough to be flushed
    // thb:当cache满了，发送map[workerINFO]的消息
    if (workerMessageSize >= maxMessagesSizePerWorker) {
      PairList<Integer, VertexIdMessages<I, M>> workerMessages = removeWorkerMessages(workerInfo);
      WritableRequest writableRequest = new SendWorkerMessagesRequest<I, M>(workerMessages);
  ...
  public static class TargetVertexIdIterator<I extends WritableComparable>
    ...
    public TargetVertexIdIterator(Vertex<I, ?, ?> vertex) {
      edgesIterator = ((Vertex<I, Writable, Writable>) vertex).getEdges().iterator();
    }
   ...
   */
  public void flush() {
    PairList<WorkerInfo, PairList<Integer, VertexIdMessages<I, M>>> remainingMessageCache = removeAllMessages();
    PairList<WorkerInfo, PairList<Integer, VertexIdMessages<I, M>>>.Iterator iterator = remainingMessageCache
        .getIterator();
    while (iterator.hasNext()) {
      iterator.next();
      WritableRequest writableRequest = new SendWorkerMessagesRequest<I, M>(
