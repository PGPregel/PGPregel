...
public class SimplePageRankComputation
  ...
  // public static int MAX_SUPERSTEPS = 20;// = GiraphConstants.MAX_PAGERANK_ITERATION.get(getConf());
  // public static double MIN_PAGERANK_PRESITION = 0.15;// = GiraphConstants.MIN_PAGERANK_PRESITION.get(conf);
  // public static double MAX_PAGERANK_PRESITION = 500;// = GiraphConstants.MAX_PAGERANK_PRESITION.get(conf);
  
  

  // public static int MAX_SUPERSTEPS = GiraphConstants.MAX_PAGERANK_ITERATION.get(conf);
  // public static double MIN_PAGERANK_PRESITION = GiraphConstants.MIN_PAGERANK_PRESITION.get(conf);
  // public static double MAX_PAGERANK_PRESITION = GiraphConstants.MAX_PAGERANK_PRESITION.get(conf);
  ...
  public void compute(
    ...
    if (getSuperstep() >= 1) {
      ...
      double value = (0.15f) + 0.85f * sum;
      
      double MIN_PAGERANK_PRESITION = getConf().getFloat("minPagerankPresition", 0.15f);
      double MAX_PAGERANK_PRESITION = getConf().getFloat("maxPagerankPresition", 500.0f);
      value = Math.max(MIN_PAGERANK_PRESITION, value);
      value = Math.min(MAX_PAGERANK_PRESITION, value);

      DoubleWritable vertexValue = new DoubleWritable(value);
      vertex.setValue(vertexValue);
      aggregate(MAX_AGG, vertexValue);
      aggregate(MIN_AGG, vertexValue);
      aggregate(SUM_AGG, new LongWritable(1));
      LOG.debug(vertex.getId() + ": PageRank=" + vertexValue +
          " max=" + getAggregatedValue(MAX_AGG) +
          " min=" + getAggregatedValue(MIN_AGG));
    }

    if (getSuperstep() < getConf().getInt("pagerankIteration", 20)) {
