    ...
    for (String fileName : giraphConf.getYarnLibJars().split(",")) {
      if (fileName.length() > 0) {
        ...
        LOG.info("Adding " + fileName + " to LocalResources for export.to " +
          filePath);
        if (fileName.contains("geo-pregel-core")) {
          coreJarFound = true;
        }
        addFileToResourceMap(map, fs, filePath);
      }
    }
    if (!coreJarFound) { // OK if you are running geo-pregel-examples-jar-with-deps
      LOG.warn("Job jars (-yj option) didn't include geo-pregel-core.");
    }
