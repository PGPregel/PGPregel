...
public class SendOneMessageToManyCache<I extends WritableComparable, M extends Writable>
  ...
  public SendOneMessageToManyCache(ImmutableClassesGiraphConfiguration conf,
    ...
    if (GiraphConstants.PRIVACY_LABEL_PER_WORKER.isDefaultValue(conf))
      throw new IllegalStateException("privacy per worker not be set");

    // split the input
    String[] privacy_per_worker_split = GiraphConstants.PRIVACY_LABEL_PER_WORKER.get(conf).split(" ");
    // check whether the num of privacy value equal to worker.
    // numWorkers inlucde the master, exclude it.
    if (privacy_per_worker_split.length != numWorkers - 1 && serviceWorker.getWorkerInfo().getTaskId() != -1)
      throw new IllegalStateException(
          "Illegal privacy per worker input:" + GiraphConstants.PRIVACY_LABEL_PER_WORKER.get(conf)
    ...
    sampling_rate = GiraphConstants.SAMPLING_RATE.get(conf);

    noise_budget = GiraphConstants.PRIVACY_BUDGET.get(conf) / (GiraphConstants.ITERATION.get(conf) - 1)
        / (noise_combiner_cnt == 0 ? 1 : noise_combiner_cnt);

    noise_budget = Math.log(noise_budget / sampling_rate + 1);

    deltaf = GiraphConstants.PAGERANK_UPPER_BOUND.get(conf) - GiraphConstants.PAGERANK_LOWER_BOUND.get(conf);

    LOG.info("Privacy: iteration -->" + GiraphConstants.ITERATION.get(conf));
    LOG.info("Privacy: PAGERANK_UPPER_BOUND -->" + GiraphConstants.PAGERANK_UPPER_BOUND.get(conf));
    LOG.info("Privacy: PAGERANK_LOWER_BOUND -->" + GiraphConstants.PAGERANK_LOWER_BOUND.get(conf));
